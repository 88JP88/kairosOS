...
if($filter=="filter"){
        ...
        if($param=="placeIdCarSites"){
            $query = mysqli_query($conectar, "SELECT s.siteId, s.clientId, s.infoSite, s.placeId,JSON_EXTRACT(p.infoPlace, '$[0].info.name') AS name FROM generalSites s JOIN generalPlaces p ON p.placeId = s.placeId WHERE s.clientId = '$clientId' AND s.placeId = '$value' and JSON_EXTRACT(s.infoSite, '$.params.siteType')= 'site'");
