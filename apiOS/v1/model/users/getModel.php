...
if($filter=="filter"){
        ...
        if($param=="placeId"){
            $query = mysqli_query($conectar, "SELECT s.siteId, s.clientId, s.infoSite, s.placeId, JSON_UNQUOTE(JSON_EXTRACT(p.infoPlace, '$[0].info.name')) AS name FROM generalSites s JOIN generalPlaces p ON p.placeId = s.placeId WHERE s.clientId = '$clientId' AND s.placeId IN (SELECT placeId FROM generalPlaces WHERE clientId = '$clientId' AND JSON_UNQUOTE(JSON_EXTRACT(infoPlace, '$.info.name')) LIKE '%$value%')");
